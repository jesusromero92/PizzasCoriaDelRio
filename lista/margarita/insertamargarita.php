...
if (!isset($_REQUEST['enviar1'])){
	...
	if ($mysqli=mysqli_connect("localhost","root")){
		if($mysqli->select_db("test")){
			$query='insert into registro values (NULL, "'.$_REQUEST['nombre'].'", "'.$_REQUEST['calle'].'", "'.$_REQUEST['telefono'].'", "'.$_REQUEST['postal'].'", "'.@$_REQUEST['domicilio'].'", "'.@$_REQUEST['pequeña'].'", "'.@$_REQUEST['mediana'].'", "'.@$_REQUEST['Familiar'].'")';
			$resultado=$mysqli->query($query);
			if($resultado)
				$mensaje="Su pedido ha sido realizado.La pizza estará horneada en 15 minutos";
			else 
