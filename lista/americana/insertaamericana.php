...
if (!isset($_REQUEST['enviar2'])){
	...
	if ($mysqli=mysqli_connect("localhost","root")){
		if($mysqli->select_db("test")){
			$query='insert into registro2 values (NULL, "'.$_REQUEST['nombre'].'", "'.$_REQUEST['calle'].'", "'.$_REQUEST['telefono'].'", "'.$_REQUEST['postal'].'", "'.@$_REQUEST['domicilio'].'", "'.@$_REQUEST['pequeña'].'", "'.@$_REQUEST['mediana'].'", "'.@$_REQUEST['Familiar'].'")';
